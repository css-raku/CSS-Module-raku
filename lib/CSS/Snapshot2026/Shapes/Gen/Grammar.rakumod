unit grammar CSS::Snapshot2026::Shapes::Gen::Grammar;
#| shape-image-threshold: <opacity-value>
rule decl:sym<shape-image-threshold> { :i ("shape-image-threshold") ":" <val(/<prop-val-shape-image-threshold> /, &?ROUTINE.WHY)>}
rule prop-val-shape-image-threshold { :i <opacity-value> }
#| shape-margin: <length-percentage [0,∞]>
rule decl:sym<shape-margin> { :i ("shape-margin") ":" <val(/<prop-val-shape-margin> /, &?ROUTINE.WHY)>}
rule prop-val-shape-margin { :i <length-percentage> }
#| shape-outside: none | [ <basic-shape> || <shape-box> ] | <image>
rule decl:sym<shape-outside> { :i ("shape-outside") ":" <val(/<prop-val-shape-outside> /, &?ROUTINE.WHY)>}
rule prop-val-shape-outside { :i [none & <keyw> ] || [[<basic-shape> :my $*A;<!{
    $*A++
}>|| <shape-box> :my $*B;<!{
    $*B++
}>]+] || <image>  }
#| <basic-shape> = <basic-shape-rect> | <circle()> | <ellipse()> |  <polygon()> | <path()> | <rect()>
rule basic-shape { :i <basic-shape-rect> || <circle> || <ellipse> || <polygon> || <path> || <rect>  }
...
rule rect { :i "rect(" [[<length-percentage> || [auto & <keyw> ] ] ** 4% ","? [[round & <keyw> ] <border-radius> ] ?  || <usage(&?ROUTINE.WHY)> ] ")" }
...
rule polygon { :i "polygon(" [<prop-val-fill-rule> ? [[[round & <keyw> ] <length> ] ","]? [<length-percentage> <length-percentage> ] +% ","?  || <usage(&?ROUTINE.WHY)> ] ")" }
#| path( <'fill-rule'>? , <string> )
rule path { :i "path(" [[<prop-val-fill-rule> ","]? <string>  || <usage(&?ROUTINE.WHY)> ] ")" }
#| shape( <'fill-rule'>? from <position> , <shape-command>#)
rule shape { :i "shape(" [<prop-val-fill-rule> ? [from & <keyw> ] <position> "," <shape-command> +% ","?  || <usage(&?ROUTINE.WHY)> ] ")" }
