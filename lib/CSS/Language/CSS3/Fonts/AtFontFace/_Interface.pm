use v6;
# generated by gen-properties.pl --role=CSS::Language::CSS3::Fonts::AtFontFace::_Interface etc/css3x-font-@fontface-properties.txt
role CSS::Language::CSS3::Fonts::AtFontFace::_Interface {
    method annotation($/) { ... }
    method character-variant($/) { ... }
    method common-lig-values($/) { ... }
    method contextual-alt-values($/) { ... }
    method discretionary-lig-values($/) { ... }
    method east-asian-variant-values($/) { ... }
    method east-asian-width-values($/) { ... }
    method family-name($/) { ... }
    method feature-tag-value($/) { ... }
    method feature-value-name($/) { ... }
    method font-face-name($/) { ... }
    method format($/) { ... }
    method historical-lig-values($/) { ... }
    method numeric-figure-values($/) { ... }
    method numeric-fraction-values($/) { ... }
    method numeric-spacing-values($/) { ... }
    method ornaments($/) { ... }
    method string($/) { ... }
    method styleset($/) { ... }
    method stylistic($/) { ... }
    method swash($/) { ... }
    method urange($/) { ... }
    method uri($/) { ... }
}
